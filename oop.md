...
_Связывание (binding)_ - это сопоставление вызванного метода с конкретной реализацией (участком памяти), которая будет исполнена.

_Early binding (static/compile-time)_ - связывание происходит на этапе компиляции. Компилятор может точно определить, какой метод вызвать по типу переменной, т.е. по типу ссылки на объект. Используется в следующих случаях:

- вызов `static`, `private` или `final` метода
- вызов переопределенного (overloaded) метода
- обращение к полю объекта

```java
class Animal {
    static void staticMethod() { System.out.println("Animal static"); }
    private void privateMethod() { System.out.println("Animal private"); }
    final void finalMethod() { System.out.println("Animal final"); }
    void say(int a) { System.out.println("Animal says " + a); }
    void say(String s) { System.out.println("Animal says " + s); }
}
...
Animal a = new Dog();
a.staticMethod(); // "Animal static"
a.say("hello"); // "Animal says hello" - compiler chose the implementation
```

_Late binding (dynamic/runtime)_ - связывание происходит во время выполнения программы по типу объекта, а не по типу ссылки. Данный вид связывания используется при вызове переопределенных (overridden) методов. Late binding обеспечивает полиморфизм.
...
Вызов методов при late binding происходит через vtable (virtual method table). _vtable_ - это таблица указателей на реализации методов, которую JVM создает для каждого класса на этапе компиляции. Обращение происходит по индексу к таблице класса объекта, на котором был вызван метод.

Важно: vtable не является частью спецификации JVM, это конкретная реализация динамической диспетчеризации (dynamic dispatch) в большинстве JVM.
