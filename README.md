...
- [В чём заключается разница между методами `Thread.start()` и `Thread.run()`?](concurrency.md#в-чём-заключается-разница-между-методами-threadstart-и-threadrun)
...
- [Чем отличается работа метода `wait()` с параметрами и без параметра?](concurrency.md#чем-отличается-работа-метода-wait-с-параметрами-и-без-параметра)
...
- [Чем отличаются интерфейсы `Runnable` и `Callable`?](concurrency.md#чем-отличаются-интерфейсы-runnable-и-callable)
